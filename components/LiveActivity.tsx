...
const LiveActivity: React.FC = () => {
  ...
  const [recentActivity, setRecentActivity] = useState<{ type: 'call' | 'chat'; purchase: RecentPurchase } | null>(null);
  ...
  useEffect(() => {
    ...
    const activityInterval = setInterval(() => {
      ...
      setRecentActivity({ type, purchase: { name, plan } });
  ...
  // Effect to hide the activity pop-up after a delay
  useEffect(() => {
    let timer: number;
    if (showActivity) {
  ...
  const activityPopup = (activity: { type: 'call' | 'chat'; purchase: RecentPurchase }) => (
    <div className={`absolute -bottom-5 w-full flex justify-center px-2 transition-all duration-500 ease-out ${showActivity ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
        <p className={`text-xs backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-lg text-center truncate ${activity.type === 'call' ? 'bg-cyan-800/80 text-white' : 'bg-teal-800/80 text-white'}`}>
            {activity.purchase.name} ने अभी {activity.purchase.plan} का {activity.type === 'call' ? 'कॉल' : 'चैट'} प्लान खरीदा।
        </p>
    </div>
  );
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-slate-700/50 p-6 rounded-xl shadow-lg border border-cyan-500/30 grid grid-cols-1 md:grid-cols-2 gap-4 divide-y md:divide-y-0 md:divide-x divide-slate-600/50">
            ...
            <div className="relative flex items-center justify-center space-x-4 py-4 md:py-0 md:pr-4">
              ...
              {recentActivity?.type === 'call' && activityPopup(recentActivity)}
            </div>
            ...
            <div className="relative flex items-center justify-center space-x-4 py-4 md:py-0 md:pl-4">
              ...
              {recentActivity?.type === 'chat' && activityPopup(recentActivity)}
            </div>
          </div>
        </div>
      </div>
